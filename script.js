...
const DEF = {
  glass: {
    "Классическое прозрачное (зеленоватая кромка), 10 мм": { trap: 12000, rect: 9500 },
    "Осветлённое Crystal Vision (без оттенка), 10 мм": { trap: 15000, rect: 12000 },
    "Тонированное графит (серое), 10 мм": { trap: 13500, rect: 11500 },
    "Тонированное бронза (коричневое), 10 мм": { trap: 13500, rect: 11500 },
    "Триплекс 5+5 мм, классическое прозрачное": { trap: 16000, rect: 13500 },
    "Триплекс 6+6 мм, классическое прозрачное": { trap: 18000, rect: 15000 },
    "Триплекс осветлённое 6+4, Crystal Vision": { trap: 21000, rect: 17500 },
    "Триплекс осветлённое 6+6, Crystal Vision": { trap: 23000, rect: 19000 },
    "Триплекс графит серое 5+5 мм": { trap: 19000, rect: 16000 },
    "Триплекс графит серое 6+6 мм": { trap: 21000, rect: 17500 },
    "Триплекс бронза 5+5 мм": { trap: 19000, rect: 16000 },
    "Триплекс бронза 6+6 мм": { trap: 21000, rect: 17500 }
  },
  hard: [
    { name: "Точечные крепления + соединительные коннекторы", price: 1800, unit: "шт" },
    { name: "Опорный профиль h100 мм", price: 6500, unit: "м.пог" },
    { name: "Соединительные коннекторы", price: 900, unit: "шт" }
  ],
  rail: [
    { name: "Деревянный поручень 40×40, масло с воском", price: 2800 },
    { name: "Деревянный поручень 40×40, эмаль однотонная (белый/чёрный)", price: 3200 },
    { name: "Деревянный поручень 40×40, покрытие по образцу заказчика", price: 3800 },
    { name: "Алюминиевый поручень 40×40", price: 2500 },
    { name: "Алюминиевый П-профиль 15×15 на верхнюю кромку стекла", price: 1500 }
  ],
  services: [
...
function buildRailSelect() {
  ...
  D.rail.forEach(item => {
    const o = document.createElement('option');
    o.value = item.name;
    o.textContent = item.name + ' — ' + fmt(item.price) + ' ₽/м.пог';
    s.appendChild(o);
  });
  if ([...s.options].some(o => o.value === prev)) s.value = prev;
}
...
function calc() {
  ...
  const glassSum = trapA * g.trap + rectA * g.rect;
  ...
  let railSum = 0;
  if (railManual !== null) railSum = railManual;
  else if (railLength !== null) railSum = railLength * railItem.price;
  
  if (railSum > 0 || (railLength !== null && railLength > 0)) {
  ...
  document.querySelectorAll('.servPrice').forEach(inp => {
    ...
    if (v !== null && v > 0) {
      servSum += v;
      servLines.push(`${s.name} — ${rub(v)}`);
    } else if (s.emptyDefault === 'free') {
      servLines.push(`${s.name} — бесплатно`);
    }
  });

  const delSum = el('delOn').checked ? num('delPrice') : 0;
  ...
  const base1_4 = glassSum + hardSum + railSum;
  let instSum = 0;
  if (el('instOn').checked) {
    instSum = mode === 'fix' ? num('instFix') : base1_4 * num('instPct') / 100;
  }

  const rawTotal = glassSum + hardSum + railSum + delSum + instSum + servSum;
  const total = roundUp500(rawTotal);
  
  el('sum').textContent = rub(total);

  if (!termManual) {
    el('termDays').value = /триплекс/i.test(gName) ? M.termTripl : M.termGlass;
  }
  
  el('termHint').textContent = termManual
    ? 'Срок задан вручную'
    : (/триплекс/i.test(gName) 
        ? 'Авто: триплекс — ' + M.termTripl + ' раб. дней' 
        : 'Авто: стекло 10 мм — ' + M.termGlass + ' раб. дней');
  ...
  t += ` — ${rub(hardSum + railSum)}\n`;
  t += `3. Доставка, разгрузка — ${delSum > 0 ? rub(delSum) : 'не требуется'}\n`;
  t += `4. Монтажные работы — ${instSum > 0 ? rub(instSum) : 'не требуются'}\n`;
